...
type IInsurancePartnerRepository interface {
	...
	GetAllPrivateProfiles() ([]models.PrivatePartnerProfile, error)
...
func (r *InsurancePartnerRepository) GetAllPrivateProfiles() ([]models.PrivatePartnerProfile, error) {
	query := `
		SELECT
			-- ========== PUBLIC INFORMATION ==========
			-- A. Brand Identification Information
	...
	var profiles []models.PrivatePartnerProfile
	err := r.db.Select(&profiles, query)
	if err != nil {
		slog.Error("Error getting all private profiles: ", "error", err)
		return nil, fmt.Errorf("internal server error")
	}

	if profiles == nil {
		profiles = []models.PrivatePartnerProfile{}
	}
