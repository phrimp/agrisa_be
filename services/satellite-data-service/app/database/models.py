...
class SatelliteImage(Base):
    ...
    analysis_results = relationship(
        "ImageAnalysis", back_populates="satellite_image", cascade="all, delete-orphan"
    )
...
class ImageAnalysis(Base):
    ...
    satellite_image_id = Column(
        String(15), ForeignKey("satellite_images.id"), nullable=False
    )

    # Analysis metadata
    analysis_type = Column(
        String(100), nullable=False
    )  # e.g., "NDVI", "Land_Classification", "Change_Detection"
    ...
    result_geometry = Column(
        Geometry("MULTIPOLYGON", srid=4326)
    )  # Spatial results if applicable
...
class CollectionImage(Base):
    ...
    collection_id = Column(
        String(15), ForeignKey("satellite_collections.id"), primary_key=True
    )
...
class DownloadRequest(Base):
    ...
    expires_at = Column(
        Integer
    )  # When the download expires (Unix timestamp in seconds)
